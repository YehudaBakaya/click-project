...
const SignUpScreen = ({ onSignUp, navigation }) => {
  ...
  const [googleRequest, googleResponse, promptGoogleAsync] =
    Google.useAuthRequest({
      clientId:
        "868613634026-boge0msthqseaskve4genkviiv3ps50j.apps.googleusercontent.com", // Web Client ID
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      // שליחת בקשה לשרת
      const response = await axios.post(`${SERVER_URL}/singUpUser`, {
        mail,
        password,
      });
      ...
      console.log("המשתמש נרשם בהצלחה:", user.email);

      // שמירת userToken ב-AsyncStorage
